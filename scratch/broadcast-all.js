...
async function main() {
  console.log("=== BROADCASTING WELCOME EMAIL TO ALL STUDENTS ===");
  ...
  console.log(`Found ${emails.length} student emails in the database.`);
...
See you there — let's start building! 🚀`;
  ...
  for (let i = 0; i < emails.length; i += batchSize) {
    ...
    if (res.ok) {
      ...
      console.log(`Batch ${Math.floor(i / batchSize) + 1} sent. Resend ID:`, data.id);
    } else {
      console.error(`Failed to send batch starting at index ${i}:`, data.message);
    }
  }

  console.log(`=== BROADCAST COMPLETED: ${successCount} emails sent successfully! ===`);
}
