...
async function main() {
  console.log("=== BROADCASTING TO REMAINING STUDENTS ===");
  
  const { data: students, error } = await supabase.from('registrations').select('email');
  if (error) {
  ...
  // Batch 1 (0-49) and Batch 2 (50-99) were sent successfully.
  // Batch 4 (150-158) was sent successfully (index 150 to end).
  // ONLY Batch 3 (index 100 to 149) failed.
  const remainingEmails = emails.slice(100, 150);
  console.log(`Sending to Batch 3: ${remainingEmails.length} students (index 100 to 149).`);
...
See you there — let's start building! 🚀`;
  ...
  if (res.ok) {
    console.log("SUCCESS: Remaining Batch 3 sent. Resend ID:", data.id);
  } else {
    console.error("FAILED to send remaining Batch 3:", data.message);
  }
}
